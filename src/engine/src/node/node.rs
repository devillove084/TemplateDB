...
use super::{fs, tenant::Tenant};
use crate::{
    error::{Error, Result},
    lsm::{proto::version_edit::File as VersionEditFile, store_trait::Store},
    node::proto::{
        request_union, response_union, AllocateFileNamesRequest, AllocateFileNamesResponse,
        BatchRequest, BatchResponse, BeginBulkLoadRequest, BeginBulkLoadResponse,
        CommitBulkLoadRequest, CommitBulkLoadResponse, CreateBucketRequest, CreateBucketResponse,
        CreateTenantRequest, CreateTenantResponse, DescribeBucketRequest, DescribeBucketResponse,
        DescribeTenantRequest, DescribeTenantResponse, RequestUnion, ResponseUnion, TenantOptions,
    },
};
...
impl Node {
    pub async fn open(path: impl Into<PathBuf>) -> Result<Self> {
        ...
        let inner = NodeInner::new(lsm_store);
        Ok(Self {
            inner: Arc::new(inner),
        })
    }
...
impl Node {
    ...
    async fn handle_commit_bulk_load(
        ...
        for desc in req.files {
            let version_edit = VersionEditFile {
