...
use bumpalo::Bump;
use crossbeam::epoch::{pin, unprotected, Guard};

use super::{
    data::{Index, IndexItem, Key, Value},
    datapage::{DataPageBuilder, SplitPageBuilder, TypedPageRef},
    env::Env,
    error::{Error, Result},
    indexpage::IndexPageBuilder,
    iter::{ForwardIter, MergingIterBuilder, OptionIter, SliceIter},
    node::{
        ConsolidateDataIter, DataNodeIter, IndexNodeIter, Node, PageAddr, PageView, Range,
        NULL_INDEX, ROOT_INDEX,
    },
...
use crate::bwtree::{datapage::DataPageRef, indexpage::IndexPageRef};
    ...
    fn try_insert<'a: 'g, 'g>(
        ...
        loop {
            ...
            match self.table.cas(node.id, page.next(), page.into()) {
                Ok(_) => {
                    if page.len() as usize >= self.opts.data_node_delta_length {
                        node.view = page.into();
...
impl<E: Env> Tree<E> {
    ...
    fn reconcile_split_node<'a: 'g, 'g>(
        ...
        if page.len() as usize >= self.opts.index_node_delta_length {
