...
use runtime::stream::store::{
    db::stream_db::StreamDb, opt::DbOption, server::Server as StoreServer,
};
...
async fn main() -> Result<()> {
    ...
    let dbpath = "target/db/";
    let dbend = "0.0.0.0:21718";
    let db = StreamDb::open(dbpath, opt)?;
    bootstrap_service(dbend, db).await?;
