...
use crate::{
    segment_request_union, segment_response_union,
    stream::common::{
        error::{Error, Result},
        Sequence,
    },
    Command, GetSegmentRequest, HeartbeatRequest, ObserverState, Role, SealSegmentRequest,
    SegmentDesc, SegmentRequest, SegmentRequestUnion, StreamDesc,
};
...
pub(crate) mod tests {
    ...
    use crate::{
        stream::{
            ...
            common::{error::Result, Sequence},
            master::tests::build_master,
        },
        Command, CommandType, ObserverState, Role, SegmentState,
    };
    ...
    async fn default_heartbeat(stream: &crate::stream::client::node::stream::Stream) -> Result<()> {
        let observer_meta = ObserverMeta {
            observer_id: "1".to_owned(),
            writer_epoch: 0,
