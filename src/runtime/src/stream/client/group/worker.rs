...
use super::{
    ...
    stream::{EventChannel, StreamMixin},
    timer::MonoTimer,
};
use crate::stream::{
    client::{
        group::stream::ActiveLauncher, node::stream::Stream as MasterStream,
        store::transport::Transport, EpochState,
    },
        ...
        for action in self.channel.take_actions() {
            match action {
                ...
                Action::Remove { stream_id, sender } => {
                    // FIXME(luhuanbing) need transfer leadership!
