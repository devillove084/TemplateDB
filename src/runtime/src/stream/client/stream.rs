...
use super::{
    group::EventChannel, node::stream::Stream as StreamClient, policy::Policy as ReplicatePolicy,
    reader::stream::StreamReader, store::transport::Transport, Engine,
};
use crate::{stream::common::error::Result, StreamDesc};
...
impl From<i32> for Role {
    fn from(role: i32) -> Self {
        match crate::Role::from_i32(role) {
            Some(crate::Role::Follower) | None => Role::Follower,
            Some(crate::Role::Leader) => Role::Leader,
        }
    }
}
