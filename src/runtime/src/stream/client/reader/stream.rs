...
use crate::stream::{
    client::{
        node::stream::Stream as MasterStream, policy::Policy as ReplicatePolicy,
...
impl StreamReader {
    pub(crate) fn new(
        ...
        StreamReader {
            ...
            transport,
            master_stream,
            // FIXME(luhuanbing) provide a mechanism to start reading from the beginning of a stream
...
impl StreamReader {
    ...
    pub async fn try_next(&mut self) -> Result<Option<Box<[u8]>>> {
        // TODO(luhuanbing) Implementing a try operation is not easy, because the underlying
