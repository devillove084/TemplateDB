...
use crate::stream::{
    client::policy::{GroupReader, Policy as ReplicatePolicy},
...
mod tests {
    ...
    fn replicate_acked_index_if_all_entries_are_replicated() {
        ...
        assert!(
            matches!(&pending_writes[0].kind, MutKind::Write(write) if write.entries.is_empty()
&& write.acked_seq.index == 1023)
        );
    }
