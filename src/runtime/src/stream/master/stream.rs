...
impl ThresholdSwitching {
    fn new() -> Self {
        // TODO(luhuanbing) support size option.
        ThresholdSwitching {}
    }
...
impl StreamInfo {
    pub fn new(parent_id: u64, stream_id: u64, stream_name: String) -> Self {
        // TODO(luhuanbing) support configuring switch policy.
        StreamInfo {
            parent_id,
            stream_id,
            stream_name,
...
impl StreamInner {
    ...
    pub fn observe(&mut self, observer_id: &str, observer_info: ObserverInfo) -> bool {
        ...
        if let Some(segment_info) = self.segments.get_mut(&acked_epoch) {
            // TODO(luhuanbing) update all previous epochs.
    ...
    pub fn gen_promote_cmd(&self, observer_id: &str) -> Command {
        // TODO(luhuanbing) set pending epochs.
...
fn apply_strategies(
    ...
    if applicant.epoch < stream_info.epoch {
        ...
        // it for idempotent.
        // info!("stream {} epoch {} leader {} promote observer {}, epoch: {}, by receiving staled
        // heartbeat",     stream_id, stream_info.epoch, stream_info.leader_desc(),
