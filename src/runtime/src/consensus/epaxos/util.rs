...
use super::types::{Command, SharedInstance};

pub(crate) async fn instance_exist<C: Command + Clone>(
    instance: &Option<SharedInstance<C>>,
) -> bool {
    if instance.is_some() {
...
pub(crate) async fn recv_message<M: DeserializeOwned>(conn: &mut TcpStream) -> M {
