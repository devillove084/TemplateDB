...
impl<R: ReleaseReferringLogFile> PartialStream<R> {
    ...
    fn commit_write_txn(
        ...
        self.stabled_tables
            .insert(*active_log_number, replace(active_mem_table, delta_table));
    ...
    /// A ['None'] if returned if there no more ready entries.
    pub fn scan_entries(
        &self,
        segment_epoch: u32,
        first_index: u32,
    ...
    pub fn is_end_of_epoch_reached(
        &self,
        target_epoch: u32,
        require_acked: bool,
        ...
        let (seq, entry) = match next {
            ...
            None if !require_acked => {
                // All entries are read, do not wait more entrires
                return true;
            }
            None => {
                return false;
