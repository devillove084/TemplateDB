        ...
        match join_type {
            JoinType::Inner | JoinType::Left => {
                ...
                let left_filter_indices = PrimitiveArray::<UInt64Type>::from(
                    compute::filter(&left_indices, predicate)?.to_data().clone(),
                );
                let right_filter_indices = PrimitiveArray::<UInt32Type>::from(
                    compute::filter(&right_indices, predicate)?
                        .to_data()
                        .clone(),
                );
    ...
    pub async fn execute(self) {
        ...
        for batch in self.right_child {
            ...
            match self.join_type {
                ...
                // TODO(luhuanbing): For every item on the left and right we check if it matches
