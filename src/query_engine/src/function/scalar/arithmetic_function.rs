...
use arrow::array::*;
use arrow::compute::kernels::numeric::{add, div, mul, neg, sub};
...
impl SubtractFunction {
    fn subtract(inputs: &[ArrayRef]) -> Result<ArrayRef, FunctionError> {
        ...
        let left = &inputs[0];
        let right = &inputs[1];
        binary_primitive_array_op!(left, right, sub)
    }

    fn negate_interval(input: &ArrayRef) -> Result<ArrayRef, FunctionError> {
        match input.data_type() {
            DataType::Interval(IntervalUnit::YearMonth) => {
                compute_op!(input, neg, IntervalYearMonthArray)
            }
            DataType::Interval(IntervalUnit::DayTime) => {
                compute_op!(input, neg, IntervalDayTimeArray)
            }
            other => Err(FunctionError::InternalError(format!(
...
impl MultiplyFunction {
    fn multiply(inputs: &[ArrayRef]) -> Result<ArrayRef, FunctionError> {
        ...
        let left = &inputs[0];
        let right = &inputs[1];
        binary_primitive_array_op!(left, right, mul)
    }
...
impl DivideFunction {
    fn divide(inputs: &[ArrayRef]) -> Result<ArrayRef, FunctionError> {
        ...
        let left = &inputs[0];
        let right = &inputs[1];
        binary_primitive_array_op!(left, right, div)
    }
