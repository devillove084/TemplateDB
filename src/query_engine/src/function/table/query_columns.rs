...
impl QueryColumnsFunc {
    fn generate_query_engine_tables_names() -> Vec<String> {
        vec![
    ...
    fn generate_query_engine_tables_types() -> Vec<LogicalType> {
        vec![
            LogicalType::Varchar,
            LogicalType::Varchar,
            LogicalType::Varchar,
    ...
    fn bind_func(
        ...
        let data = QueryColumnsData::new(
            entries,
            Self::generate_query_engine_tables_types(),
            Self::generate_query_engine_tables_names(),
        );
        return_types.extend(data.return_types.clone());
        return_names.extend(data.return_names.clone());
        Ok(Some(FunctionData::QueryColumnsData(Box::new(data))))
    }
    ...
    fn tables_func(
        ...
        if let Some(FunctionData::QueryColumnsData(data)) = input.bind_data {
    ...
    pub fn register_function(set: &mut BuiltinFunctions) -> Result<(), FunctionError> {
        set.add_table_functions(TableFunction::new(
            "query_engine_columns".to_string(),
            Some(Self::bind_func),
            Self::tables_func,
        ))?;
        Ok(())
