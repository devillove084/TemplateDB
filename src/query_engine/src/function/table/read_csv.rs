...
impl ReadCSV {
    ...
    fn create_reader(input: ReadCSVInputData) -> Result<Reader<File>, FunctionError> {
        ...
        if let Some((bounds_start, bounds_end)) = new_bounds {
            if let Some(proj) = input.projection {
                let reader = ReaderBuilder::new(input.schema)
                    .with_batch_size(input.option.read_batch_size)
                    .with_bounds(bounds_start, bounds_end)
                    .with_header(input.option.has_header)
                    .with_projection(proj)
                    .with_delimiter(input.option.delimiter)
                    .build(file);
                return Ok(reader?);
            };
        };
        Err(FunctionError::ArrowError(
