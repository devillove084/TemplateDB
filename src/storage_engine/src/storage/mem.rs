...
impl MemStorage {
    ...
    fn is_ok_to_create<P: AsRef<Path>>(&self, name: P) -> TemplateResult<()> {
        if let Some(p) = name.as_ref().parent() {
            ...
            // root file
            Err(TemplateKVError::IO(IOError::new(
                ErrorKind::AlreadyExists,
...
impl Storage for MemStorage {
    ...
    fn open<P: AsRef<Path>>(&self, name: P) -> TemplateResult<Self::F> {
        ...
        match self.inner.read().unwrap().get(&path) {
            Some(n) => match n {
                Node::Dir => Err(TemplateKVError::IO(IOError::new(
                    ErrorKind::NotFound,
                    format!("{}: Try to open a directory", &path),
                ))),
                Node::File(f) => Ok(f.clone()),
            },
            None => Err(TemplateKVError::IO(IOError::new(
                ErrorKind::NotFound,
        ...
        if recursively {
            ...
            if let Some(n) = map.get(&key) {
                match n {
                    ...
                    Node::File(_) => Err(TemplateKVError::IO(IOError::new(
                        ErrorKind::NotFound,
