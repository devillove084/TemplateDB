    ...
    pub fn decode_from(src: &[u8]) -> TemplateResult<(Self, usize)> {
        if let Some((offset, n)) = VarintU64::read(src) {
            if let Some((size, m)) = VarintU64::read(&src[n..]) {
                ...
                Err(TemplateKVError::Corruption("bad block handle".to_owned()))
            }
        } else {
            Err(TemplateKVError::Corruption("bad block handle".to_owned()))
        }
...
mod tests {
    ...
    use super::{
        ...
        table::{new_table_iterator, Table, TableBuilder, TableIterator},
    };
    use crate::{
        ...
        memtable::{
            batch::WriteBatch,
            key_format::{InternalKey, InternalKeyComparator, ParsedInternalKey},
            memtable::MemTable,
    ...
    impl Comparator for ReverseComparator {
        ...
        fn name(&self) -> &str {
            "TemplateDB.ReverseBytewiseComparator"
        }
    ...
    struct KeyConvertingIterator<I: Iterator> {
        inner: I,
        err: Cell<Option<TemplateKVError>>,
    }
    ...
    impl<I: Iterator> Iterator for KeyConvertingIterator<I> {
        ...
        fn key(&self) -> &[u8] {
            match ParsedInternalKey::decode_from(self.inner.key()) {
                ...
                None => {
                    self.err.set(Some(TemplateKVError::Corruption(
                        "malformed internal key".to_owned(),
                    )));
    ...
    struct DBConstructor {
        inner: TemplateDB<MemStorage, TestComparator>,
    }

    struct DBIterWrapper {
        inner: TemplateDBIterator<MemStorage, TestComparator>,
        key_buf: Vec<u8>,
        value_buf: Vec<u8>,
    }
    impl DBIterWrapper {
    ...
    impl Constructor for DBConstructor {
        ...
        fn new(is_reversed: bool) -> Self {
            ...
            let db = TemplateDB::open_db(options, "table_testdb", env).expect("could not open db");
            Self { inner: db }
        }
