...
use runtime::{
    mutate_request_union, mutate_response_union, store_client, MutateRequest, MutateRequestUnion,
    MutateResponse, ReadRequest, ReadResponse, SealRequest, SealResponse, WriteRequest,
    WriteResponse,
};
...
impl StoreClient {
    pub async fn read(
        &self,
        input: ReadRequest,
    ) -> crate::runtime_client::Result<Streaming<ReadResponse>> {
        let mut client = self.client.clone();
...
mod tests {
    ...
    async fn reject_staled_sealing_request() -> crate::runtime_client::Result<()> {
        ...
        match client.seal(1, 2, SealRequest { segment_epoch: 1 }).await {
            Err(crate::runtime_client::Error::Staled(_)) => {}
            _ => {
