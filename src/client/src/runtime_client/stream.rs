...
use runtime::StreamDesc;
use tokio::sync::mpsc;

use super::{
    group::EventChannel, node::stream::Stream as StreamClient, policy::Policy as ReplicatePolicy,
    reader::stream::StreamReader, store::transport::Transport, Engine, Result,
};
...
impl From<i32> for Role {
    fn from(role: i32) -> Self {
        match runtime::Role::from_i32(role) {
            Some(runtime::Role::Follower) | None => Role::Follower,
            Some(runtime::Role::Leader) => Role::Leader,
        }
    }
}
