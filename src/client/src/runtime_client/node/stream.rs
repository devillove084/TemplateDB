...
use runtime::{
    segment_request_union, segment_response_union, stream::Sequence, Command, GetSegmentRequest,
    HeartbeatRequest, ObserverState, Role, SealSegmentRequest, SegmentDesc, SegmentRequest,
    SegmentRequestUnion, StreamDesc,
};

use super::client::MasterClient;
use crate::runtime_client::{Error, Result};
...
pub(crate) mod tests {
    ...
    use runtime::{stream::build_master, Command, CommandType, ObserverState, Role, SegmentState};

    use crate::runtime_client::{
        node::{
            stream::{ObserverMeta, SegmentDesc},
            Master,
        },
        Result, Sequence,
    };
    ...
    async fn default_heartbeat(stream: &crate::runtime_client::node::stream::Stream) -> Result<()> {
        let observer_meta = ObserverMeta {
            observer_id: "1".to_owned(),
            writer_epoch: 0,
