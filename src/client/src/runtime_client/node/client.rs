...
use runtime::{
    node_client, segment_request_union, segment_response_union, stream_request_union,
    stream_response_union, tenant_request_union, tenant_response_union, HeartbeatRequest,
    HeartbeatResponse, SegmentRequest, SegmentRequestUnion, SegmentResponse, StreamRequest,
    StreamRequestUnion, StreamResponse, TenantRequest, TenantRequestUnion, TenantResponse,
};
...
impl MasterClient {
    pub fn new(chan: Channel) -> Self {
        let client = node_client::NodeClient::new(chan);
        Self { client }
    }
}
