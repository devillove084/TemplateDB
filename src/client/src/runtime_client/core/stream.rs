...
use crate::runtime_client::{
    core::message::MsgDetail, policy::Policy as ReplicatePolicy, Entry, EpochState, Error, Result,
    Role, Sequence,
};
...
impl StreamStateMachine {
    ...
    pub fn propose(&mut self, event: Box<[u8]>) -> Result<Sequence> {
        if self.role == Role::Follower {
            Err(Error::NotCommandLeader(self.leader.clone()))
        } else {
