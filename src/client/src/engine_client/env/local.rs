...
use engine::{
    error::Result,
    file::store_trait::SequentialWrite,
    lsm::{
        iterator::MergingIterator,
        table::format::{Key, ValueType},
    },
    node::{
        bucket::Bucket,
        node::Node,
        proto::{BatchRequest, BatchResponse},
        tenant::Tenant,
    },
};
...
impl super::Env for Env {
    ...
    async fn tenant(&self, name: &str) -> Result<Self::TenantEnv> {
        let tenant = self.node.tenant(name).await?;
        Ok(TenantEnv { tenant })
    }

    async fn handle_batch(&self, req: BatchRequest) -> Result<BatchResponse> {
        self.node.handle_batch(req).await
    }
}
...
impl Iter for LocalIter {
    ...
    async fn seek(&mut self, target: &[u8]) -> Result<()> {
        let key = Key::encode_to_vec(target, u64::MAX, ValueType::Put);
