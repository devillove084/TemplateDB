macro_rules! impl_trait {
    ($self:ident) => {
        ...
        fn into_marked_ptr(
            self,
        ) -> crate::debra::reclaim::pointer::MarkedPtr<Self::Item, Self::MarkBits> {
        ...
        fn marked(
            $self: Self,
            tag: usize,
        ) -> crate::debra::reclaim::pointer::Marked<Self::Pointer> {
            let inner = $self.inner.with_tag(tag);
            crate::debra::reclaim::pointer::Marked::Value(Self {
                inner,
                _marker: PhantomData,
            })
        }
        ...
        fn decompose($self: Self) -> (Self, usize) {
            ...
            (
                Self {
                    inner: crate::debra::reclaim::pointer::MarkedNonNull::from(inner),
                    _marker: PhantomData,
                },
                tag,
            )
