    ...
    V: Copy,
{
    pub(super) fn insert(
        ...
        loop {
                    ...
                    .unwrap_or(false)
            {
                ...
                let v = Box::new(val);
                let old = rn.val.swap(Box::into_raw(v), Ordering::SeqCst);
                // drop(new_node);
            ...
            if unsafe { &*left_node }
                .next
                .compare_and_swap(right_node, new_node_ptr, Ordering::SeqCst)
                == right_node
            {
                return None;
            }
    ...
    pub(super) fn delete(
        ...
        //get value to return
        let rn = unsafe { &*right_node };
        let old = unsafe { *rn.val.load(Ordering::SeqCst) };
...
mod tests {
    ...
    fn more_linked_list_tests() {
        ...
        // println!("{:?}", new_linked_list.head.load(Ordering::SeqCst));
        // new_linked_list.print();
